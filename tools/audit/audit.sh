...
if [[ ! -f "contracts/contract.schema.json" ]]; then
  ...
  if ls contracts/agents/*.json 1> /dev/null 2>&1; then
    ...
    if ajv validate -s contracts/contract.schema.json -d "contracts/agents/*.json"; then
  ...
  if ls contracts/repositories/*.json 1> /dev/null 2>&1; then
    ...
    if ajv validate -s contracts/contract.schema.json -d "contracts/repositories/*.json"; then
  ...
  if ls contracts/runners/*.json 1> /dev/null 2>&1; then
    ...
    if ajv validate -s contracts/contract.schema.json -d "contracts/runners/*.json"; then
